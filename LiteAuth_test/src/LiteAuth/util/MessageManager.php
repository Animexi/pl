...
class MessageManager {
    ...
    /**
     * Отправить сообщение с рамкой (boxed message)
     * @param mixed $player
     * @param string $title Заголовок
     * @param array $lines Строки сообщения
     */
    public function sendBoxedMessage($player, $title, $lines = array()) {
        $borderTop = "§8╔══════════════════════════════╗";
        $borderBottom = "§8╚══════════════════════════════╝";
        $side = "§8║";
        
        $player->sendMessage($borderTop);
        $player->sendMessage($side . " " . $title);
    ...
    /**
     * Отправить простое сообщение с префиксом
     * @param mixed $player
     * @param string $message
     */
    public function sendMessage($player, $message) {
        if ($player instanceof \pocketmine\Player || $player instanceof \pocketmine\command\CommandSender) {
            $player->sendMessage($message);
        }
    }
